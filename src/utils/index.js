...
export const getMax = (arr) => {
  var _max = Math.abs(parseFloat(arr[0]))
  for (var i = 0, m = arr.length; i < m; i++) {
    if (!_max) {
      _max = Math.abs(parseFloat(arr[i]))
    }
    if (Math.abs(parseFloat(arr[i])) > _max) {
      _max = Math.abs(parseFloat(arr[i]))
    }
  }
  return _max
...
export const setAxisInterval = (max) => {
  const r = Math.pow(10, max.toFixed(0).length - 1)
  return Math.ceil(max / r) * r / 4 || 0.25
}
